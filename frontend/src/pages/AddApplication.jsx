...
function ScorePie({ score }) {
  ...
  return (
    <div className="flex items-center gap-6">
      <div className="relative h-36 w-36 rounded-full" style={pieStyle}>
        <div className="absolute inset-4 rounded-full bg-white flex items-center justify-center">
          <div className="text-center">
            <p className="text-3xl font-black text-teal-700">{safeScore}%</p>
            <p className="text-[11px] text-slate-400 uppercase tracking-widest">Match</p>
          </div>
        </div>
      </div>
      <div className="space-y-2">
        <p className="text-sm text-slate-600">
          Resume and job profile comparison complete.
        </p>
        <p className="text-xs text-slate-500">
          Higher score means stronger fit for the role requirements.
        </p>
      </div>
    </div>
  );
...
export default function AddApplication() {
  ...
  const uploadResumeAndMatch = async () => {
    ...
    try {
      ...
      const resumeRes = await fetch(`${import.meta.env.VITE_BACKEND_URL}/resume/parse`, {
        method: "POST",
        body: resumeForm,
        credentials: "include",
      });
      ...
      const matchRes = await fetch(`${import.meta.env.VITE_BACKEND_URL}/match`, {
        method: "POST",
        credentials: "include",
  ...
  const saveApplication = async (status) => {
    ...
    try {
      ...
      const payload = {
        status,
        company: jobDetails.company ?? jobDetails.companyName ?? "Unknown Company",
        ...
        eligibilitySnapshot: {
          ...
          experience: extractedEligibility.experience ?? jobDetails.experience ?? null,
    ...
    <div className="min-h-screen bg-slate-50 px-4 md:px-10 py-8">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Add Application</h1>
            <p className="text-slate-500 mt-1">
              Upload job and resume PDFs to extract details, calculate match score, and save.
            </p>
          </div>
          <button
        ...
        <div className={cardClass}>
          <h2 className="text-lg font-semibold text-slate-800">1. Upload Application PDF</h2>
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <button
              ...
              disabled={loadingJob}
              className="px-5 py-2.5 rounded-xl bg-teal-700 text-white font-medium hover:bg-teal-800 disabled:opacity-60"
            >
          ...
          {jobDetails && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 rounded-xl bg-slate-50 p-4 border border-slate-100">
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">Company</p>
                <p className="text-sm font-medium text-slate-800">{jobDetails.company || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">Role</p>
                <p className="text-sm font-medium text-slate-800">{jobDetails.role || jobDetails.position || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">Location</p>
                <p className="text-sm font-medium text-slate-800">{jobDetails.location || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">Stipend/Salary</p>
                <p className="text-sm font-medium text-slate-800">{jobDetails.salary || jobDetails.stipend || "-"}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className={cardClass}>
          <h2 className="text-lg font-semibold text-slate-800">2. Upload Resume & Match</h2>
          <div className="flex flex-col md:flex-row gap-3">
            <input
              type="file"
              accept="application/pdf"
          ...
          {matchResult && (
            <div className="rounded-2xl border border-slate-100 p-4 bg-white">
              ...
              {!!matchResult.summary && (
                <p className="mt-4 text-sm text-slate-700">{matchResult.summary}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className={cardClass}>
          <h2 className="text-lg font-semibold text-slate-800">3. Save Application</h2>
          ...
          {resumeDetails && (
            <p className="text-xs text-slate-400">Resume parsed successfully.</p>
          )}
        </div>
      </div>
    </div>
